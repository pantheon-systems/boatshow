  ...
  Drupal.behaviors.boatshowCustomFilters = {
    attach: function attach(context, settings) {
      ...
      $('.brick--type--exposed-filters').once('boatshowCustomFilters').each(function () {
        ...
        var $form = $(this).closest('.brick--type--exposed-filters.collapsible');

        $filtersParent.find('input[type="radio"][name="field_date_radios"]').change(function () {
          var value = $(this).val();
          var minTarget = "#edit-field-smnr-seminar-date-value-min";
          var maxTarget = "#edit-field-smnr-seminar-date-value-max";
        ...
        if ($filtersParent.hasClass('collapsible')) {
          ...
          var openFiltersByDefault = $view.find('.views-col').length >= 6 || // Open by default if there are more than 6 results
          ...
          $filtersParent.find('.form--inline >.form-item').each(function () {
            if ($(this).css('display') != 'none' && openFiltersByDefault) {
              $(this).attr('open', '');
            }
          });
...
//# sourceMappingURL=featuresFilter.js.map
