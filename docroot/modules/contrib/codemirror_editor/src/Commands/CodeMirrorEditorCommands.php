...
class CodeMirrorEditorCommands extends DrushCommands {
  ...
  public function downloadCodemirror() {
    ...
    $source_base_path = CodeMirrorLibraryBuilder::CDN_URL . CodeMirrorLibraryBuilder::CODEMIRROR_VERSION;
    $destination_base_path = DRUPAL_ROOT . CodeMirrorLibraryBuilder::LIBRARY_PATH;

    $assets = array_merge($cm_library['js'], $cm_library['css']);
    foreach ($assets as $asset) {

      if (empty($asset['type']) || $asset['type'] != 'external') {
        $source = $source_base_path . '/' . explode(ltrim(CodeMirrorLibraryBuilder::LIBRARY_PATH, '/'), $asset['data'])[1];
